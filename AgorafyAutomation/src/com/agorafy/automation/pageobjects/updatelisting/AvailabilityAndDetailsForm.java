...
public class AvailabilityAndDetailsForm extends Page 
{

    WebElement element;
    public AvailabilityAndDetailsForm(WebDriver driver) 
    {
        super(driver);
    }
    ...
    public MediaForm clickSaveAndContinueOnDetailsForm() 
    {
        MediaForm media = null;
        try 
        {
            btn_SaveAndContinueOnDetailsForm().click();
            media = new MediaForm(driver);
            AutomationLog.info("Successfully clicked save and continue btn on details form");
        }
        catch (Exception e) 
        {
            AutomationLog.error("Failed to click save and continue btn on details form");
        }
        return media;
    }
